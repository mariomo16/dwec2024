/*
    ...
    ---------------------------------------
    Notas de la Versión: 
    - Las naves ya no siguen la misma ruta


    Meteorito cada X tiempo con alerta antes de aparecer
    + vida a los enemigos
    boss final (con armas?)

    menú lateral juego
    modal a la derecha para mostrar los controles del juego (con gif de persona hablando?)
    botón jugar muestra una ventana para elegir la dificultad y la skin de la nave
    subir dificultad a los enemigos

    Al pausar al principio, los enemigos se siguen generando (1 encima de otro)
...
ventanaModal.innerHTML = `
    ...
    <form>
        ...
        <br />
        <input type="button" id="leaderboard" value="Ver puntuaciones" class="izquierda" disabled/>
        <input type="button" id="cambiarSkin" value="Cambiar diseño de la nave" disabled/>
        <input type="button" id="startGame" value="Empezar partida" class="derecha"/>
    </form>
	...
	for (let i = 0; i < numeroEnemigos; i++) {
		...
		setTimeout(() => {
			...
			coordenadasEnemigos.push({
				...
				velocidadX: Math.random() * 3 + 1, // Velocidad aleatoria en X
				velocidadY: Math.random() * 3 + 1, // Velocidad aleatoria en Y
			});
		}, 300 * i);
	}
}
...
function mover() {
	// Establecer los límites del área de juego
	const maximoX = window.innerWidth; // Ancho de la ventana del navegador
	const maximoY = window.innerHeight; // Altura de la ventana del navegador
	...
	for (let i = 0; i < navesEnemigas.length; i++) {
		...
		if (
			coordenadas.x < 0 ||
			coordenadas.x > maximoX - navesEnemigas[i].offsetWidth
		) {
			coordenadas.dx *= -1; // Invierte la dirección en el eje X
		}
		if (
			coordenadas.y < 0 ||
			coordenadas.y > maximoY - navesEnemigas[i].offsetHeight
		) {
