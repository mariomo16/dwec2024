/*
    ...
    - Al colisionar contra un enemigo, la nave del jugador explotara (gif)
    - Corregido error de velocidad (jugador, misiles, enemigos) dependiendo del navegador
    - Corregido error al moverse mientras se mantenía pulsado el espacio (ya no dispara al moverse)
    - El usuario puede elegir si jugar o no en pantalla completa (en vez de ser automático al iniciar)
    - Las naves enemigas explotaran al impactar un misil
...
const audioLanzarMisil = new Audio("./sfx/sfx_laser1.ogg"); // Sonido al lanzar un misil
const audioPerderPartida = new Audio("./sfx/sfx_lose.ogg"); // Sonido al perder una partida
const audioImpactarEnemigo = new Audio("./sfx/"); // Sonido al impactar un enemigo
...
if (navigator.userAgent.includes("Chrome")) {
	velocidadJugador = 5;
	velocidadMisiles = 8;
	randomSpeed = (Math.random() + 2) * 1.7;
} else if (navigator.userAgent.includes("Firefox")) {
	velocidadJugador = 12;
	velocidadMisiles = 20;
	randomSpeed = (Math.random() + 6) * 1.7;
}
	...
	document.addEventListener(
		...
		(e) => {
			...
			if (e.key === " ") {
				...
				crearElementoIMG.style.top = `${naveJugador.offsetTop / 1.05}px`;
				crearElementoIMG.style.left = `${
					naveJugador.offsetLeft + naveJugador.offsetWidth / 2
				}px`;
				/*
                ...
                */
				...
				audioLanzarMisil.play(); // Reproduzco el audio al lanzar un misil
			}
		},
		5,
	);
...
function comprobarColision() {
	...
	for (let i = 0; i < navesEnemigas.length; i++) {
		if (
			...
			naveJugador.setAttribute("src", "./images/pixelbomb.gif"); // Gif explosion
	...
	for (let i = 0; i < misiles.length; i++) {
		for (let j = 0; j < navesEnemigas.length; j++) {
			if (
				...
				navesEnemigas[j].setAttribute("src", "./images/pixelbomb.gif");
